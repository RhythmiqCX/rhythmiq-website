...
| `education`    | `retro-study`                                   | `retro-study`    | `gradient`   | `#ff4d00`    |
...
- `retro-study` (education / exam prep) — sibling of `retro-vibe` for a competitive-exam revision brand: same fixed neo-brutalist palette (orange/blue/neon/cream/near-black), but the chrome is written for a study brand (subjects, revision, "the feed") instead of a kitchen, and the subject section renders up to FOUR cards instead of three. Fixed identity (`theme.accent` ignored). **Photo slots:** `hero.photo` → framed hero; `services[0..3].photo` → the four subject cards (GS / Polity / Science / English); `photos[0..3]` → the Instagram grid. `business.tagline` → hero headline (split on the first comma, second half in orange); `business.oneLiner` → hero sub; `business.about` → "Why aspirants follow" story; `signature` → pull-quote; `highlights` → marquee; `services[].tag` → a coloured badge ("Popular"/"Hot Topic"/etc). CTA is social-first — an `socials.instagram` link renders "Message on Instagram".
