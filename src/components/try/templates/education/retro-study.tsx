...
/**
 * Education / Exam-prep · Retro / neo-brutalist ("STUDY") — a 70s-meets-Gen-Z
 * landing page for a competitive-exam revision brand: cream canvas, thick 3px
 * black borders, hard offset drop-shadows, Syne display caps, Playfair italic
 * accents, Space Grotesk body, film grain. Bold and loud on purpose, aimed at
 * young aspirants who live on Instagram.
 *
 * It's a sibling of restaurant/retro-vibe: same fixed neo-brutalist identity,
 * but the chrome is written for a study brand (subjects, revision, the feed)
 * instead of a kitchen, and the subject section renders up to FOUR cards
 * (GS / Polity / Science / English) instead of three.
 *
 ...
 *   services[].photo   → the four "What we cover" subject cards
 *   photos[0..3]       → the Instagram grid
 *
 ...
 *   services (title/blurb/tag/photo) → subjects · socials.instagram → IG.
 */
...
/** "STUDY SPHERE" → STUDY ✱ SPHERE with an orange star between words. */
      ...
      <div className="mx-auto grid max-w-[1280px] items-center gap-12 px-5 py-16 sm:px-8 md:grid-cols-[1.05fr_0.95fr] md:py-24">
        ...
        <div className="relative">
          ...
          <div className="mt-9 flex flex-wrap gap-4">
            ...
            <a href="#subjects" className="retro-press no-underline uppercase" style={{ fontFamily: SYNE, fontWeight: 700, fontSize: 15, letterSpacing: "0.03em", color: DARK, background: CREAM, border: border(3), padding: "15px 30px" }}>
              See the subjects
            </a>
          </div>
        </div>
...
function Marquee({ data }: { data: Prospect }) {
  const items =
    data.highlights && data.highlights.length
      ...
      : ["60-sec revisions", "GS · Polity · Science · English", "Daily practice", "Exam-ready notes"];
...
function Subjects({ data }: { data: Prospect }) {
  const items = data.services.slice(0, 4);
  if (!items.length) return null;
  return (
    <section id="subjects" style={{ background: CREAM }}>
      <div className="mx-auto max-w-[1280px] px-5 py-16 sm:px-8 md:py-24">
        <div className="mb-10 flex flex-col gap-3">
          ...
          <h2 className="m-0 uppercase" style={{ fontFamily: SYNE, fontWeight: 800, fontSize: "clamp(34px, 5vw, 62px)", lineHeight: 0.95, letterSpacing: "-0.045em", color: DARK }}>
            What we <span style={{ fontFamily: PLAYFAIR, fontStyle: "italic", fontWeight: 400, textTransform: "none", color: ORANGE }}>cover</span>
          </h2>
        </div>
      ...
      <div className="mx-auto max-w-[1280px] px-5 py-16 sm:px-8 md:py-24">
        <div className="mb-9 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h2 className="m-0 mt-2 uppercase" style={{ fontFamily: SYNE, fontWeight: 800, fontSize: "clamp(26px, 4vw, 48px)", lineHeight: 0.95, letterSpacing: "-0.03em", color: DARK, wordBreak: "break-word" }}>
              {handle}
            </h2>
          </div>
          {ig && (
    ...
    <footer id="footer" style={{ background: DARK, color: CREAM, borderTop: `6px solid ${ORANGE}` }}>
      <div className="mx-auto grid max-w-[1280px] gap-12 px-5 py-16 sm:px-8 md:grid-cols-[1.4fr_1fr_1fr] md:py-20">
        ...
        <div style={{ fontFamily: SPACE, fontSize: 14.5 }}>
          <div className="mb-3 uppercase" style={{ fontFamily: SYNE, fontWeight: 700, fontSize: 13, letterSpacing: "0.1em", color: NEON }}>Stay in the loop</div>
          {s?.instagram && (
            <a href={s.instagram} target="_blank" rel="noopener" className="mb-2 block no-underline" style={{ color: CREAM }}>{igHandle(s.instagram)}</a>
          )}
          <div className="mb-2" style={{ color: "rgba(253,249,240,0.6)", lineHeight: 1.5 }}>DM us for notes, doubts and daily revision.</div>
      ...
      <div className="mx-auto flex max-w-[1280px] flex-col gap-2 px-5 py-6 uppercase sm:flex-row sm:justify-between sm:px-8" style={{ borderTop: "1px solid rgba(253,249,240,0.16)", fontFamily: SPACE, fontSize: 12, letterSpacing: "0.06em", color: "rgba(253,249,240,0.55)" }}>
        ...
        <span>Prototype by Rhythmiq — made to help you <span style={{ color: NEON }}>revise</span></span>
      </div>
    </footer>
  );
}
...
const EducationRetroStudy = ({ data }: { data: Prospect }) => (
  <div className="retro-study" style={{ background: CREAM, color: DARK, minHeight: "100svh", overflowX: "clip", position: "relative" }}>
    {/* film grain overlay */}
...
export default EducationRetroStudy;
